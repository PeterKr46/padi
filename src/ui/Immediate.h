...
namespace padi {
    ...
    size_t Immediate::draw(padi::UIContext *ctx, const std::shared_ptr<Animation>& anim, sf::FloatRect const& bound, uint8_t frame, sf::Color color = sf::Color::White) {
    ...
    size_t Immediate::drawScalable(padi::UIContext *ctx, const std::shared_ptr<Animation>& anim, sf::FloatRect const& bound, uint8_t frame, sf::Color color = sf::Color::White) {
        if(ctx->m_vbo.getVertexCount() < ctx->m_numVerts + 9*4) {
